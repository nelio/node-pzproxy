...
class PzProxy{
    constructor(opts = {}){
        ...
        this.logAccess = opts.logAccess || PzProxy.logAccess;
    ...
    flowRequest(req,res) {
        ...
        PzProxy._debug(req,`Proxying ${req.url} to ${req.backendURL}`);
    ...
    static logAccess(req, res, length, flags = []) {
        const timeSpent = new Date().getTime() - req.xConnectDate.getTime();

        process.stdout.write(req.xRemoteAddr + (req.xDirectRemoteAddr ? '/'
            + req.xDirectRemoteAddr : '') + ' - ' + req.xRequestID + ' [' + req.xConnectDate.toISOString() + '] "'
            + req.method + ' ' + (req.originalURL || req.url) + ' -> '+ req.backendURL
