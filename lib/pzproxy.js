...
function _flowRequest(req,res) {
	...
	return self.cache.check(req.cacheKey,req.cacheTTL,function(err,isCached){
		if ( err ) {
			self._debug(req,"Error checking if item '"+cacheKey+"' is in cache. Please fix it: ",err);
			res.writeHead(500,{'content-type':'text/plain'});
			res.write('500.1 - Cache check error');
			return self._finishRequest(req,res,{});
		}
		...
		if ( isCached ) {
			...
			return self.cache.get(req.cacheKey,function(err,resLine,stream){
				if ( err ) {
					self._debug(req,"Item '"+cacheKey+"' is cached but got an error while trying to get it: ",err);
					res.writeHead(500,{'content-type':'text/plain'});
