...
function _flowRequest(req,res) {
	...
	return self.cache.check(req.cacheKey,req.cacheTTL,function(err,isCached){
		...
		if ( req.cacheKey && req.cacheTTL > 0 ) {
			...
			return self.proxy.proxyRequest(req,res,req.backendURL,null,{},{cache:self.cache,cacheKey:req.cacheKey,timeout:self.defaultTimeout},function(err,req,res,preq,pres,infos){
