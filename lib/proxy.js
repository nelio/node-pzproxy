...
function proxyRequest(req,res,hostOrURL,port,opts,callback){
    ...
    if ( _opts.timeout ) {
        timeout = setTimeout(function(){
            ...
            res.answer(504,{'Content-type':'text/plain; charset=UTF-8'},'504 - Gateway timeout :-(');
            return callback ? callback(null,req,res,preq,null,{docSize: 25, flags: ['PT']}) : null;
        },_opts.timeout*1000);
    }

    // If client disconnects, abort the request
    req.on('close',function(){
        if ( !_canHandleRequest() )
            return;
        preq.abort();

        return callback ? callback(null,req,res,preq,null,{docSize: null, flags: ['Pd']}) : null;
    });
    ...
    preq.on('response',function(pres){
        ...
        if ( self.cache && self.opts.shouldCache(req,res,preq,pres) ) {
            ...
            cacheItem = self.cache.add(opts.cacheKey || opts.url,req.cacheTTL);
    ...
    preq.on('error',function(e){
        ...
        res.answer(503,{'content-type':'text/plain; charset=UTF-8'},msg);
        self._debug(req,"Error performing HTTP request: ",e.toString());
        return callback ? callback(null,req,res,preq,null,{docSize: msg.length, flags: ['PE']}) : null;
    });
