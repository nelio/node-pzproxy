...
function proxyRequest(req,res,hostOrURL,port,opts,callback){
    ...
    if ( _opts.timeout ) {
        timeout = setTimeout(function(){
            ...
            res.answer(504,{'Content-type':'text/plain; charset=UTF-8'},'504 - Gateway timeout :-(',['PaT']);
    ...
    if ( self.opts.abortOnClientDisconnect ) {
        req.on('close',function(){
            if ( !_canHandleRequest() )
                return;
            preq.abort();
            return callback ? callback(null,req,res,preq,null,{docSize: null, flags: ['Pad']}) : null;
        });
    }
    ...
    preq.on('error',function(e){
        ...
        res.answer(503,{'content-type':'text/plain; charset=UTF-8'},msg,['PaE']);
